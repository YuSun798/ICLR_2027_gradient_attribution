...
\caption{Accuracy-based validation. Blue points are the seven per-seed accuracy differences (gradient minus random, percentage points) on the two discrete-option tasks; green is the seven-seed mean with its 95\% interval $[-2.91, +4.80]$\,pp. The interval spans zero, so the masks are indistinguishable on accuracy, and it reaches past the 4\,pp non-inferiority margin, so that criterion is not met. Per-seed values in Appendix~\ref{app:accuracy_seeds}.}
...
using accuracy. Accuracy is reported only for Medical and Legal, the two
tasks with discrete-option outputs, over the same seven seeds.
...
detectable accuracy difference under this evaluation.
