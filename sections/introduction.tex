...
\textbf{Density-matched evidence on loss-based forgetting.} Under strict equal-parameter control, Random-B achieves lower loss-based backward transfer (Loss-based BWT) in all seven seeds, a pattern consistent with a stability--plasticity trade-off under the tested sequence. On final task accuracy for the discrete-option benchmarks, however, the 95\% confidence interval spans zero and extends to 4.80 percentage points in Grad-B's favour, so the pre-specified 4-point non-inferiority criterion is not met. The attribution pass is therefore not justified by the loss-based evidence, while its accuracy consequences remain inconclusive at $n=7$. Removing it eliminates 64 calibration forward--backward steps, an overhead equal to 12.8\% of the 500-step training budget (11.3\% of the combined 564-step count), together with the calibration-data requirement.
