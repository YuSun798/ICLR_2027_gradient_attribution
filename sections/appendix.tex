...
\vspace{2pt}
{\footnotesize $^{\dag}$\emph{Full-parameter} attribution configuration (\texttt{--include-frozen}, scoring all 3{,}085{,}938{,}688 base parameters); no method in this paper uses this configuration. The attribution actually used in the continual-learning experiments has a transient backward peak of $\sim$0.73\,GB (4-bit NF4, seq 512, 64 calibration samples, gradient checkpointing enabled; \texttt{overhead\_mb} = 699.43\,MiB over the base model), and contributes $0$ for random-B-only masking by construction (Table~\ref{tab:cl_3b}). \\ $^{\ddag}$Zero by construction (no attribution pass is run), not a measured value.}
\end{table}
...
\begin{table}[h]
\caption{Confounded comparison (Qwen2.5-3B, rank-16 LoRA, 5 tasks, $n{=}5$ seeds). Incremental cost is the attribution pass's transient peak ($\sim$0.73\,GB; $0$ for random by construction; Appendix~\ref{app:attribution_cost}). Across all tables, bold marks the better value in its column.}
\label{tab:cl_3b}
\centering
\small
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Method} & \textbf{BWT $\downarrow$} & \textbf{Loss $\downarrow$} & \textbf{Incremental cost} \\
\midrule
Shared random & $\mathbf{0.428} \pm 0.019$ & $\mathbf{1.940} \pm 0.021$ & 0 \\
Gradient mask & $0.437 \pm 0.061$ & $1.966 \pm 0.037$ & $\sim$0.73\,GB \\
No mask & $0.484 \pm 0.047$ & $1.970 \pm 0.033$ & 0 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\caption{Three distinct quantities that are sometimes all called run-to-run \emph{noise}. Only (a) is random error against a paired comparison; (b) and (c) are systematic offsets. They are not three estimates of one noise magnitude.}
...
\midrule
(a) within-session contrast noise & execution path only & $9.6 \times 10^{-5}$ & paired-comparison error; the only term the significance tests use \\
(b) position (ordering) offset & position of the two arms & $0.064$ (seed 42, \texttt{gradient\_B}) & systematic; a position effect, not random error \\
(c) cross-session absolute offset & session and hardware & $0.120$ (\texttt{full\_alora}, seed 42) & systematic; shifts the absolute BWT level between sessions \\
\bottomrule
\end{tabular}
\end{table}
...
paths that should agree do agree.
...
the within-run contrast survives it at all three seeds.
...
Gradient-B@10\% (\texttt{gradient\_B}) & --- & --- & 0.453 & 0.423 \\
...
$9.6\times10^{-5}$. The level-(c) spread is a property of the absolute BWT
level of a session, not a random perturbation of the comparison. We therefore
report it as \emph{cross-session variation in the absolute BWT level} and do not
...
Both cross-architecture designs are tabulated together below: the confounded design (gradient selecting B-only, random selecting A+B) and the within-\texttt{lora\_B} design (both methods restricted to \texttt{lora\_B}). The two designs are not comparable row-for-row because the Phi-2 within-B rows use a different LoRA target set; \S\ref{sec:boundaries} discusses both.

\begin{table}[t]
\caption{Cross-architecture comparison on Qwen2.5-3B and Phi-2 (2.78B, GPT-NeoX-style), both arms within a row sharing one target set. \emph{Top:} confounded design, gradient selecting B-only and random selecting A+B; both architectures use all linear projections (\texttt{q,k,v,fc1,fc2,dense}; pool $23{,}592{,}960$). \emph{Bottom:} within-\texttt{lora\_B} design, both methods restricted to \texttt{lora\_B}; the Phi-2 rows use a \emph{different} target set omitting \texttt{dense} (\texttt{q,k,v,fc1,fc2}; pool $20{,}971{,}520$, 12.5\% smaller), so the two designs are not comparable row-for-row. The within-\texttt{lora\_B} rows are matched on pool but \emph{not} density (the gradient mask activates approximately $2\times$ the B parameters of Random-B), as on Qwen2.5-3B.}
...
\begin{table}[t]
\caption{For accuracy, positive values indicate higher accuracy for Grad-B; for loss-based BWT, positive values indicate greater forgetting for Grad-B. Col.~2 (accuracy difference) is measured on an A800 GPU; Col.~3 (loss-based BWT difference) is measured on the same A800 GPU. Taken together they form a second environment relative to the original V100 loss runs. Col.~2 carries the 95\% $t$-interval on the seven-seed mean. Col.~3: mean $+0.152$ (SD $0.0749$, $t = 5.37$, CI $[+0.083, +0.221]$, 7/7 same direction). The abstract's $-0.155$ is the same loss contrast measured on the V100 stack under the opposite convention [Random $-$ Grad].}
\label{tab:accuracy_branch}
\centering
\small
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Seed} & \textbf{Accuracy $\Delta$ [Grad $-$ Rand]} & \textbf{Loss BWT $\Delta$ [Grad $-$ Rand]} \\
\midrule
42   & $+0.0200$ & $+0.2320$ \\
123  & $0.0000$  & $+0.1647$ \\
456  & $+0.0048$ & $+0.1300$ \\
...
Loss on math decreases with training while accuracy on the same task collapses to zero for all arms: peak accuracy at math's own training phase is $0.04$ and every arm ends at $0.00$. The cause is not a format mismatch---the numeric-match function accepts a bare number, a number with surrounding text, and a complete solution alike, and the training and evaluation prompt prefixes are identical by construction. The surviving explanation is truncation: a generation audit reports 56--95\% of generations across all tasks and arms reaching the 256-token cap without an end-of-sequence token, and for math specifically 81--90\% of mathematical generations, accumulated across the six evaluation calls per task, reach the cap the same way. The last number in a truncated generation is an intermediate quantity, so the reference answer is absent from the value the scorer inspects. Every arm shares the same cap and scorer, so the bias is common rather than arm-specific. We do not treat the math column as a forgetting result: under option-based scoring math yields no score at all, and under generation scoring it is confounded with truncation. Scoring each option by its log-likelihood removes the truncation dependence, and that is the scorer used above.
