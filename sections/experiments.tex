...
% Because the task-level contributions span roughly two orders of magnitude, the left panel uses a broken $y$-axis: Code, Legal, and Math are shown on the lower range, while Medical is shown on the upper range. 
...
forgetting on every prior task, with most of the aggregate difference
concentrated on Medical. Per-seed results are reported in Appendix~\ref{app:d1_per_task}.
}
\label{fig:per_task}
\end{figure}
...
Two rival explanations for the effect can be excluded from the same data. The gradient arm's loss on each task's own diagonal is lower than the random arm's in every seed and task, yet it forgets more, so the two arms differ in retention rather than in learning capacity (Appendix~\ref{app:d1_diagonal}). A second reading, 
% that the gradient arm would do better with its full budget, rests on a single seed: at seed 42 the density-matched gradient arm beats the unrestricted one, but at the seven-seed mean the ordering reverses.
the effect of restoring the gradient arm’s full budget is not stable across seeds: at seed 42, the density-matched gradient arm outperforms the unrestricted one, whereas the ordering reverses at the seven-seed mean.

\input{sections/accuracy_branch}
